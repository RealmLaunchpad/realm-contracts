...
10. Dev buy only: **`RealmFactory.BuyOnDeploy`** (`token, buyer=msg.sender, ethSpent, tokensBought, recipients, amounts`) — the same event and the same split rule the curve venue uses in §1.2, sourced from the pool instead of the curve. `ethSpent` is what the buy cost in the dev-buy pair's own quote: `msg.value` (wei) on a native pair, `devBuy.quoteAmount` in the quote ERC20's raw units on an ERC20 pair. The name is the curve venue's, where it is always native.
