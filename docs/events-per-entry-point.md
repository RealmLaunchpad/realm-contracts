...
7. Graduation milestone, once per token: **`RealmToken.Graduated`** (no args) right after the token's own `Transfer` out of the `PoolManager`, on the first buy that leaves the token's graduation pool (§1.3 step 7) at or past 5x its launch market cap. Emitted by whichever pool's buy the transfer settles, but decided by the graduation pool's price only. Spot-priced, so a flash-loan pump can trigger it: cosmetic by design, nothing on-chain depends on it.
...
A buy on an ERC20-quoted pool can also emit the graduation milestone **`RealmToken.Graduated`**, exactly as §6.1 step 7: after the token's `Transfer` out of the `PoolManager`, following step 4.
