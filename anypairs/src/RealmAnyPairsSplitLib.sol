...
library RealmAnyPairsSplitLib {
    ...
    /// @dev The conversion router did not consume the whole quote it was given. Redeclared on the hook so its ABI
    /// decodes it. See {swapToWeth}.
    error PartialFill();
    ...
    /// @dev AUDIT ROUND 17. TWO SIDES ARE MEASURED HERE, AND THEY ARE NOT THE SAME QUESTION -- the same pairing the
    /// trackers' payout sites carry.
    ///
    ///   (A) "How much of the hook's quote actually left?" A LEDGER question, and only the SENDER's decrease can
    ///       answer it. This site had no answer at all before round 17, and it is the ONE conversion path in the
    ///       codebase whose caller zeroes its ledger entry up front ({claimTo}) and does not re-credit on the
    ///       success branch. `SwapRouter02.exactInput` is not obliged to spend its whole `amountIn`; anything it
    ///       left behind would sit on the hook with `owed[claimer][token]` already at zero, out of reach of
    ///       `owed`, of `accruedQuote` and of `rescue` (which refuses pool assets) -- permanently. Latent only
    ///       because `swapRouter` is admin-set and V3's `exactInput` does spend in full, which is exactly the kind
    ///       of "safe for a reason that is not in this function" the round-15 fee-on-transfer defect was.
    ///       REFUSING a short spend, rather than re-crediting it, is what keeps this a one-line rule: the revert is
    ///       caught by {claimTo}, whose whole frame unwinds -- approval included -- and the claim is then paid in
    ///       the raw quote, which is the documented fallback for every other conversion failure here.
    ///
    ///   (B) "How much WETH did the claimer receive?" A PRICING question about value ARRIVING from a router, so the
    ///       RECIPIENT's increase is the correct side (the same reasoning as the trackers' conversion sites), and
    ///       it is clamped to the router's own reported output so that WETH pushed to the recipient by something
    ///       else during the swap is not counted as this claim's proceeds.
    function swapToWeth(
        address recipient,
        address token,
        bytes memory path,
        ...
        // (A) sender side, and (B) recipient side, both sampled before the call.
        uint256 quoteBefore = IERC20(token).balanceOf(address(this));
        uint256 before = IERC20(weth).balanceOf(recipient);
        uint256 reported = ISwapRouter02Lib(swapRouter).exactInput(
            ISwapRouter02Lib.ExactInputParams({path: path, recipient: recipient, amountIn: amount, amountOutMinimum: minOut})
        );
        // (A): the quote the router did not take would be stranded, because the ledger entry is already zero.
        uint256 quoteAfter = IERC20(token).balanceOf(address(this));
        uint256 spent = quoteBefore > quoteAfter ? quoteBefore - quoteAfter : 0;
        if (spent < amount) revert PartialFill();
        // (B) Saturating: a recipient that forwards WETH onward must not make a successful swap revert.
    ...
    function claimTo(
        ...
        uint256 delivered = before > aft ? before - aft : 0;
        // AUDIT ROUND 14 (F-5): RE-CREDIT what never left. The ledger is zeroed before the transfer, so a token that
        // reports success while moving nothing -- a soft blocklist -- would otherwise destroy the credit permanently.
        if (delivered < amount) owed[msg.sender][token] += amount - delivered;
