...
interface IEBExttload { function exttload(bytes32 slot) external view returns (bytes32); }

interface IEBV3Factory {
    function getPool(address tokenA, address tokenB, uint24 fee) external view returns (address pool);
}

interface IEBV3Pool { function liquidity() external view returns (uint128); }
...
interface IEBWETH9 { function deposit() external payable; }
...
contract RealmAnyPairsDividendTrackerEthBasket {
    ...
    constructor(Config memory c) {
        if (c.weth == address(0)) revert BadBasket();
        token = c.token;
        feeder = c.feeder;
        weth = c.weth;
        ...
        if (c.swapRouter != address(0)) excluded[c.swapRouter] = true; // router dust must not accrue rewards
    ...
    function basketLeg(uint256 i) external view returns (address asset, uint16 bps, bytes memory path) {
        ...
        if (asset != weth) {
            path = RealmAnyPairsRouteLib.encode(
                RealmAnyPairsRouteLib.best(swapRouter == address(0) ? address(0) : v3Factory, poolManager, weth, true, asset),
                asset
            );
        }
    }
    ...
    function process(uint256 gasBudget) public nonReentrant returns (uint256 pushed) {
        ...
        while (iterations < n) {
            ...
            if (amt != 0 && (!inLock || h.code.length == 0)) {
                ...
                catch { emit PushPaymentFailed(h, amt); }
            }
    ...
    function _claim(address to, uint256[] memory minOuts, bytes[] memory routes) internal returns (uint256 totalAmount) {
    ...
    function _requireRoutes(bytes[] memory routes) internal view {
        ...
        for (uint256 i; i < routes.length; ++i) {
            if (routes[i].length == 0 || _legAsset[i] == weth) continue;
            RealmAnyPairsRouteLib.supplied(routes[i], swapRouter == address(0) ? address(0) : v3Factory, poolManager, weth, true, _legAsset[i], feeder);
        }
    }
    ...
    function _checkRecipient(address to) internal view {
        if (
            to == address(0) || to == address(this) || to == token || to == weth || to == swapRouter
        ...
        returns (uint256 out)
    {
        ...
        RealmAnyPairsRouteLib.Route memory r = route.length == 0
            ? RealmAnyPairsRouteLib.best(swapRouter == address(0) ? address(0) : v3Factory, poolManager, weth, true, asset)
            : RealmAnyPairsRouteLib.supplied(route, swapRouter == address(0) ? address(0) : v3Factory, poolManager, weth, true, asset, feeder);
        ...
        if (r.venue == RealmAnyPairsRouteLib.VENUE_V3) {
            ...
            IEBSwapRouter02(swapRouter).exactInput(
                IEBSwapRouter02.ExactInputParams({
                    path: RealmAnyPairsRouteLib.v3Path(r, asset), recipient: to, amountIn: ethAmt, amountOutMinimum: minOut
                })
            );
            if (IERC20(weth).balanceOf(address(this)) > wethBefore) revert PartialFill();
            IERC20(weth).forceApprove(swapRouter, 0);
        } else {
            if (r.tokenIn == weth) IEBWETH9(weth).deposit{value: ethAmt}();
            _v4Swapping = true;
            IPoolManager(poolManager).unlock(
                abi.encode(to, r.tokenIn, asset, r.fee, r.tickSpacing, r.hooks, ethAmt, minOut)
            );
            _v4Swapping = false;
        }
