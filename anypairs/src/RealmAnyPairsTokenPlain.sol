...
/// @dev The hook's permissionless reserve refresh, poked by {RealmAnyPairsTokenPlain.attachTracker}.
interface IRealmAnyPairsHookTail {
...
contract RealmAnyPairsTokenPlain is ERC20 {
    ...
    /// @dev What the optional PUSH leaves behind -- the heaviest launchable coin's `minDebit` (546k stipend
    /// -> 571,015) + 60,000 margin. The optional CREDIT is deliberately NOT reserved; see
    /// {RealmAnyPairsTokenDividend.PROCESS_FLOOR} for the asymmetry and the audit-round-8 reasoning.
    uint256 private constant TAIL_RESERVE = 631_015;
    uint256 private constant PROCESS_FLOOR = 940_000; // >= PROCESS_GAS * 64/63 + pre-call work + TAIL_RESERVE
    uint256 private constant MAX_TRACKER_SYNC_GAS = 1_100_000;
    /// @dev Heaviest tracker {attachTracker} accepts: the launcher's basket cap, so a late attach can never raise the hook's
    /// global post-swap reserve (or the coins' {PROCESS_FLOOR} margin) past what launches already assume.
    uint256 private constant MAX_ATTACH_SYNC_GAS = RealmAnyPairsGasLib.MAX_TRACKER_STIPEND;
    error TrackerTooHeavy();
    /// @dev Gas cap of {attachTracker}'s `refreshCoinTail` poke (one pool-list read, a staticcall back here, one store).
    uint256 private constant REFRESH_TAIL_GAS = 100_000;
...
/// and round 9 added the hook-side per-`tx.origin` accumulator for V4 buys taken as ERC-6909 claims.
    ...
    function attachTracker(address tracker_) external {
        ...
        if (g > MAX_ATTACH_SYNC_GAS) revert TrackerTooHeavy();
        ...
        // Transfers now need `minDebit`: have the hook reserve it in multi-hop trades even if its side was wired first.
        // Best-effort (anyone can call `refreshCoinTail` later), but never starved: the floor guarantees the full cap.
        if (gasleft() < REFRESH_TAIL_GAS + REFRESH_TAIL_GAS / 63 + 5_000) revert InsufficientGasForBalanceSync();
        (bool rOk,) = h.call{gas: REFRESH_TAIL_GAS}(abi.encodeCall(IRealmAnyPairsHookTail.refreshCoinTail, (address(this))));
        rOk;
    }
    ...
    /// for the mandatory debit, credit threshold). All zero while {tracker} is zero: no transfer floor applies.
    /// @dev The tax hook reads `minDebit` to size its post-swap reserve.
    /// @dev `minNotify` is ALSO the gas a buyer's transfer needs for the buyer to be REGISTERED for rewards
    /// (audit round 8, F5 -- this is the number integrators should surface). Below it the transfer still
    /// succeeds and nothing is lost, but the recipient's tracked balance lags until their next large-gas
    /// transfer, any claim, a permissionless `syncBalance`, or a paid `distributeFor(gasBudget, toSync)`.
    function syncGasParams() external view returns (uint256 stipend, uint256 minDebit, uint256 minNotify) {
        if (tracker == address(0)) return (0, 0, 0);
        return _syncGas();
    }
    ...
    /// weight), the credit is skippable (self-heals), and the auto-push is best-effort. Only the PUSH leaves
    /// {TAIL_RESERVE} behind; see {RealmAnyPairsTokenDividend.PROCESS_FLOOR} for why the credit does not.
