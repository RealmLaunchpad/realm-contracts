...
/// @notice Puts a working `LivoKeepersRegistry` at the address the token implementations bake in, and
///         appoints `keeper`.
...
function installKeepersRegistry(address owner, address keeper) returns (LivoKeepersRegistry registry) {
    LivoKeepersRegistry deployed = new LivoKeepersRegistry(owner);
    KEEPERS_VM.etch(DeploymentAddresses.LIVO_KEEPERS_REGISTRY, address(deployed).code);
    KEEPERS_VM.label(DeploymentAddresses.LIVO_KEEPERS_REGISTRY, "KeepersRegistry");

    registry = LivoKeepersRegistry(DeploymentAddresses.LIVO_KEEPERS_REGISTRY);
