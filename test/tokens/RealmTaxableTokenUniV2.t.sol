...
/// @notice Unit tests for `RealmTaxableTokenUniV2`. Forks mainnet (via `LaunchpadBaseTests`) so the
...
contract RealmTaxableTokenUniV2Tests is LaunchpadBaseTestsWithUniv2Graduator, V2SwapHelpers {
    ...
    function test_preGraduation_blocksTransferToPair() public {
        // Pre-graduation, transfers to pair are blocked at the RealmToken gate; tax never gets a chance.
        ...
        vm.prank(buyer);
        vm.expectRevert(IRealmToken_TransferToPairBeforeGraduationNotAllowed_selector());
    ...
    function test_graduationAnchored_windowAnchoredAtGraduation() public {
        ...
        uint40 launchTs = IRealmToken(gToken).launchTimestamp();
        ...
        assertEq(
            IRealmToken(gToken).getTaxConfig().sellTaxBps, SELL_BPS, "tax active in the graduation-anchored window"
        );
        ...
        assertEq(IRealmToken(gToken).getTaxConfig().sellTaxBps, 0, "tax ends `duration` after graduation");
    }
    ...
    function test_setTaxBps_callableByLaunchpadOwner_lowersBoth() public {
        ...
        emit RealmTaxableToken.TaxBpsUpdated(newBuy, newSell);
    ...
    /// @dev Helper that returns the selector of `RealmToken.TransferToPairBeforeGraduationNotAllowed`.
    function IRealmToken_TransferToPairBeforeGraduationNotAllowed_selector() internal pure returns (bytes4) {
