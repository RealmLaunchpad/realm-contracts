...
contract DividendsGasTests is TaxTokenUniV4BaseTests {
    ...
    function _create(uint16 dividendsBps) internal returns (RealmTaxableTokenUniV4) {
        ...
        // and the dividend token has a buffer big enough to distribute when a measurement wants one.
    ...
    /// @dev Lands a fresh distribution on `token` (a no-op for a token without dividends), so the next
    ///      transfer of every holder has to settle. Fresh earnings first: a native payout is
    ///      uncapped, so the previous distribution drained the buffer.
    function _distribute(RealmTaxableTokenUniV4 token) internal {
        if (!token.hasDividends()) return;
    ...
    ///      cares about: the ONE-OFF first-ever touch of an account slot, the first transfer after a
    ///      later distribution (both slots warm, both re-checkpointed), and a transfer with nothing
    ///      distributed since, where `paid == rpt` on both sides and nothing is written.
    /// @param distributed whether to land a distribution before each of the first two transfers. IDLE
    ///        is the common case by a wide margin — the accumulator only moves when the keeper
    ///        distributes — and it is the case where the feature costs almost nothing.
    function _measure(RealmTaxableTokenUniV4 token, bool distributed)
        internal
        returns (uint256 firstEver, uint256 afterDistribution, uint256 idle)
    {
        ...
        if (distributed) _distribute(token);
        vm.prank(buyer);
        uint256 g = gasleft();
        erc.transfer(holderA, unit);
        firstEver = g - gasleft();

        // 2. A fresh distribution against warm, non-zero slots: one non-zero->non-zero SSTORE per side.
        if (distributed) _distribute(token);
        vm.prank(buyer);
        g = gasleft();
        erc.transfer(holderA, unit);
        afterDistribution = g - gasleft();

        // 3. Nothing distributed since: every account is already checkpointed at the current value and
    ...
    function test_gas_hotPathOverheadOfDividends() public {
        ...
        (uint256 pFirst, uint256 pAfter, uint256 pIdle) = _measure(plainIdle, false);

        RealmTaxableTokenUniV4 divIdle = _create(5_000);
        (uint256 iFirst, uint256 iAfter, uint256 iIdle) = _measure(divIdle, false);

        RealmTaxableTokenUniV4 divLive = _create(5_000);
        (uint256 sFirst, uint256 sAfter, uint256 sIdle) = _measure(divLive, true);

        console.log("--- wallet-to-wallet transfer, execution gas ---");
        console.log("no dividends          : firstEver / afterDistribution / idle", pFirst, pAfter, pIdle);
        console.log("dividends, idle       : firstEver / afterDistribution / idle", iFirst, iAfter, iIdle);
        console.log("dividends, distributed: firstEver / afterDistribution / idle", sFirst, sAfter, sIdle);
        ...
        // IDLE — the case that dominates. With nothing distributed the accumulator cannot have moved,
        // so the hook short-circuits on the global slot and writes nothing, ever.
        assertLt(iFirst - pFirst, 6_000, "an idle dividend token barely costs anything");
        assertLt(iIdle - pIdle, 4_000, "and keeps costing barely anything on every later transfer");

        // DISTRIBUTED, ONE-OFF. Two accounts each paying a zero->non-zero SSTORE the first time they
        // are ever settled, plus the global slot.
        assertLt(sFirst - pFirst, 60_000, "first-ever settle of two account slots");

        // DISTRIBUTED, RECURRING: the first transfer after each distribution re-checkpoints both warm
        // account slots.
        assertLt(sAfter - pAfter, 12_000, "the first transfer after a distribution");

        // IDLE AGAIN: `paid == rpt` on both sides and the hook falls through without a single write.
        assertLt(sIdle - pIdle, 4_000, "a transfer with nothing distributed since writes nothing");
    }
    ...
    /// @dev Measured as the FIRST call after a distribution, with every account slot already warm, i.e.
    ///      the worst recurring case rather than the common one: with nothing distributed since the
        ...
        returns (uint256 transferGas, uint256 buyGas, uint256 sellGas)
    {
        ...
        // one-off zero->non-zero write. The distribution before the warm-up matters: with the
        // accumulator unmoved the hook writes nothing, so a warm-up without one would not warm the
        // account slots at all and every "steady state" number below would really be a first-touch one.
        _distribute(token);
        vm.prank(buyer);
        erc.transfer(holderA, unit);
        _distribute(token);
    ...
    /// @dev Lands a distribution on EVERY configured asset, so the measurement below is the worst case:
    ///      every leg has to be settled on the next transfer. Fresh earnings first, since the native leg
    ///      is uncapped and drains its buffer each time.
    function _fundEveryAsset(RealmTaxableTokenUniV4 token) internal {
        vm.roll(block.number + 1); // the funding leg is once per block, per asset
        vm.deal(address(this), 3 ether);
    ...
    /// @dev Transfer cost as the payout set grows. This is the number the deployer has to be shown:
    ///      every extra payout asset is another per-account slot settled on both sides of the first
    ///      transfer after that asset distributes — and with staggered thresholds, something has almost
    ///      always just distributed.
    function test_gas_perExtraDividendAsset() public {
        ...
        console.log("--- first transfer after every asset distributed, execution gas ---");
    ...
    /// @dev One transfer right after every asset distributed, with every account slot already warm.
