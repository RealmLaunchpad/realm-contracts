...
/// @dev Minimal token exposing a toggleable `graduated()` flag, mirroring `IRealmToken.graduated()`.
...
/// @notice Fork-free unit tests for `RealmCreatorVault` math/guards and `RealmCreatorVaultFactory`.
///         The vesting clock starts at vault creation (init); claims are gated on `token.graduated()`.
contract RealmCreatorVaultUnitTest is Test {
    MockGraduatableToken token;
    RealmCreatorVault impl;
    ...
    function setUp() public {
        ...
        impl = new RealmCreatorVault();
    }

    function _newVault(uint256 cliff, uint256 vesting) internal returns (RealmCreatorVault vault) {
        vault = RealmCreatorVault(payable(Clones.clone(address(impl))));
    ...
    function test_vaultAcceptsNativeDividends() public {
        RealmCreatorVault vault = _newVault(CLIFF, VESTING);
    ...
    function test_rescueTokens_capFollowsWhatHasBeenClaimed() public {
        RealmCreatorVault vault = _newVault(CLIFF, 0); // full unlock at the cliff
    ...
    function test_initialize_rejectsZeroToken() public {
        RealmCreatorVault vault = RealmCreatorVault(payable(Clones.clone(address(impl))));
        vm.expectRevert(RealmCreatorVault.InvalidToken.selector);
        vault.initialize(address(0), owner, ALLOC, CLIFF, VESTING);
    }

    function test_initialize_rejectsZeroOwner() public {
        RealmCreatorVault vault = RealmCreatorVault(payable(Clones.clone(address(impl))));
        vm.expectRevert(RealmCreatorVault.InvalidOwner.selector);
        vault.initialize(address(token), address(0), ALLOC, CLIFF, VESTING);
    }

    function test_initialize_rejectsZeroAmount() public {
        RealmCreatorVault vault = RealmCreatorVault(payable(Clones.clone(address(impl))));
        vm.expectRevert(RealmCreatorVault.InvalidAmount.selector);
    ...
    function test_claim_revertsBeforeGraduation() public {
        RealmCreatorVault vault = _newVault(0, VESTING);
    ...
    function test_claim_incremental_sumsToAllocation() public {
        RealmCreatorVault vault = _newVault(0, VESTING); // no cliff
        token.setGraduated(true);
        uint256 start = block.timestamp;
