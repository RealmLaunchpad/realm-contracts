...
import {ILivoKeepersRegistry} from "src/interfaces/ILivoKeepersRegistry.sol";

/// @title KeeperGated
/// @notice The keeper check the out-of-band earnings conversions share. See `LivoKeepersRegistry` for
...
abstract contract KeeperGated {
    /// @notice The `LivoKeepersRegistry` holding the set of addresses allowed to trigger this token's
    ...
    address public constant LIVO_KEEPERS_REGISTRY = DeploymentAddresses.LIVO_KEEPERS_REGISTRY;
    ...
    function _requireKeeper() internal view {
        require(ILivoKeepersRegistry(LIVO_KEEPERS_REGISTRY).isKeeper(msg.sender), NotAKeeper());
    }
}
