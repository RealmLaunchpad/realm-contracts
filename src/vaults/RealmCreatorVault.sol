...
contract RealmCreatorVault is Initializable {
        ...
        initializer
    {
        ...
        // rejects 0-bps vaults, but `RealmCreatorVaultFactory.createVault` is permissionless).
    ...
    function claim() external {
        ...
        require(IRealmToken(token).graduated(), NotGraduated());
    ...
    function claimable() external view returns (uint256) {
        if (!IRealmToken(token).graduated()) return 0;
