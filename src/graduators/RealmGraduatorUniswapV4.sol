...
contract RealmGraduatorUniswapV4 is IRealmGraduator, Ownable {
    ...
    /// @notice The shared, permissionless `RealmUniV4LiquidityAdder` singleton — deployed once, independent
    ...
    /// @param _liquidityAdder The shared `RealmUniV4LiquidityAdder` singleton (deployed separately). All
    ...
    /// @return Address of the pool manager (same for all tokens, but to comply with the IRealmGraduator interface)
    ...
    function graduateToken(address tokenAddress, uint256 tokenAmount) external payable override {
        ...
        // the token and the graduator is NOT in `RealmTaxableToken._dividendExcluded` (adding it would put
        ...
        returns (uint128 liquidity1, uint128 liquidity2)
    {
        ...
        if (remainingEth > 0) {
            ...
            liquidity2 = IRealmUniV4LiquidityAdder(LIQUIDITY_ADDER).addSingleSidedEth{value: remainingEth}(
