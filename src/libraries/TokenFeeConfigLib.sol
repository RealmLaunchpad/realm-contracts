...
library TokenFeeConfigLib {
    ...
    struct Config {
        ...
        /// @dev Every ERC20 asset this token has ever been paid in, in first-payment order. Native
        ///      (`address(0)`) is implicit and never stored, so the native deposit path pays nothing for it. `setShares` has to snapshot each recipient's accrual across ALL of
