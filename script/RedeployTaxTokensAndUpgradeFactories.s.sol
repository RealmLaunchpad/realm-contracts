...
/// @notice Wraps up the rollout for the `setTaxBps` feature added to `RealmTaxableToken`.
...
///         it doesn't inherit `RealmTaxableToken`.
///
///         Single broadcast, four new deployments + two proxy upgrades:
///         1. `RealmTaxableTokenUniV2`
///         2. `RealmTaxableTokenUniV4`
///         3. `RealmFactoryUniV2Unified` impl wired to (1) plus unchanged non-tax pieces.
///         4. `RealmFactoryUniV4Unified` impl wired to (2) plus unchanged non-tax pieces.
...
contract RedeployTaxTokensAndUpgradeFactories is Script {
    ...
    function _getDeps() internal view returns (Deps memory d) {
        if (block.chainid == DeploymentsEthereumMainnet.BLOCKCHAIN_ID) {
            ...
            require(
                AddressesFromRealmTaxableTokenV4.BLOCKCHAIN_ID == DeploymentAddressesEthereumSepolia.BLOCKCHAIN_ID,
    ...
    function run() public {
        ...
        console.log("=== Realm Tax-Token Redeploy + Both Factory Upgrades ===");
        ...
        fresh.factoryV4Impl = address(
            new RealmFactoryUniV4Unified(
                d.launchpad,
                IRealmFactory.TokenImpls({base: d.tokenImpl, tax: fresh.taxTokenV4Impl}),
                d.bondingCurve,
                d.graduatorV4,
                d.masterFeeHandler,
                CreatorVaultScriptConfig.factoryFor(),
