...
/// @notice Deploys the `LivoKeepersRegistry`.
///
/// @dev ⚠️ DEPLOY ORDER. This must land BEFORE the taxable token implementations on a given chain: they
///      bake the address into their bytecode as `DeploymentAddresses.LIVO_KEEPERS_REGISTRY`, which today
...
///
/// @dev No proxy, unlike `LivoDividendSwapRegistry`. This contract is a mapping and two setters with no
...
///
/// Usage (dry run):  forge script DeployLivoKeepersRegistry --rpc-url sepolia --account livo.dev
/// Usage (deploy):   forge script DeployLivoKeepersRegistry --rpc-url sepolia --account livo.dev --slow --broadcast --verify
contract DeployLivoKeepersRegistry is Script {
    function run() external {
        address owner = _resolveOwner();

        console.log("=== Deploy LivoKeepersRegistry ===");
        console.log("Chain ID: %d", block.chainid);
        console.log("Owner:    %s", owner);

        vm.startBroadcast();
        address registry = address(new LivoKeepersRegistry(owner));
        vm.stopBroadcast();

        console.log("");
        console.log("LivoKeepersRegistry: %s", registry);
        console.log("");
        console.log("Next: paste it into LIVO_KEEPERS_REGISTRY in src/config/DeploymentAddresses.sol,");
