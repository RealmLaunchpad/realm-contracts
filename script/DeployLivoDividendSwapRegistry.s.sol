...
/// @notice Deploys the `LivoDividendSwapRegistry` implementation + UUPS proxy.
///
...
///
/// Usage (dry run):  forge script DeployLivoDividendSwapRegistry --rpc-url sepolia --account livo.dev
/// Usage (deploy):   forge script DeployLivoDividendSwapRegistry --rpc-url sepolia --account livo.dev --slow --broadcast --verify
contract DeployLivoDividendSwapRegistry is Script {
    function run() external {
        address owner = _resolveOwner();
        uint256 threshold = _defaultThreshold();

        console.log("=== Deploy LivoDividendSwapRegistry ===");
        console.log("Chain ID:  %d", block.chainid);
        console.log("Owner:     %s", owner);
        console.log("Threshold: %d (native 18-dec)", threshold);

        vm.startBroadcast();
        address impl = address(new LivoDividendSwapRegistry());
        address proxy =
            address(new ERC1967Proxy(impl, abi.encodeCall(LivoDividendSwapRegistry.initialize, (owner, threshold))));
        vm.stopBroadcast();

        console.log("=== Deployed ===");
        console.log("LivoDividendSwapRegistry (impl):  %s", impl);
        console.log("LivoDividendSwapRegistry (proxy): %s", proxy);
        console.log("");
    ...
    function _resolveOwner() internal view returns (address owner) {
        ...
        require(owner != address(0), "LIVO_TREASURY missing");
    }
